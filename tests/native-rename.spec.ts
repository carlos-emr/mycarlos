...
test("renames native-library folders and documents with accessible dialogs", async ({
  page,
}, testInfo) => {
  ...
  await page.addInitScript(() => {
    const snapshot = {
      profiles: [
        { id: "profile", displayName: "FAKE Patient", createdAtMs: 1 },
      ],
      folders: [
        {
          id: "folder",
    ...
    Reflect.set(window, "__TAURI_INTERNALS__", {
      invoke: async (
        ...
        if (
          command === "vault_rename_record" &&
          args?.request.recordId === "record"
        ) {
  ...
  await page.goto("/");
  await page
    .getByRole("button", { name: "Rename folder FAKE Old folder" })
    .click();
  ...
  await page.screenshot({
    path: testInfo.outputPath("rename-folder.png"),
    fullPage: true,
  });
  await folderDialog.getByRole("button", { name: "Save name" }).click();
  await expect(
    page.getByRole("article", { name: "FAKE Results folder" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Grid view" }).click();
  await page
    .getByRole("button", { name: "Rename folder FAKE Results" })
    .click();
  await page.getByRole("button", { name: "Cancel", exact: true }).click();
  await page
    .getByRole("article", { name: "FAKE Old.pdf document" })
    .getByRole("button")
    .click();
  ...
  await expect(
    page.getByRole("article", { name: "FAKE Bloodwork.pdf document" }),
  ).toBeVisible();
  const libraryAccessibility = await new AxeBuilder({ page })
    .withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa"])
    .analyze();
  ...
  await page.screenshot({
    path: testInfo.outputPath("renamed-library.png"),
    fullPage: true,
  });
});
