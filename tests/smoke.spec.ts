...
test("renders the mock-aligned patient library at each target viewport", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  ...
  await expect(
    page.getByText("Bloodwork — cholesterol and liver panel"),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Browser preview", { exact: true }).first(),
  ).toBeVisible();
});

test("imports browser-selected PDF metadata for the session", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page
    .getByRole("button", { name: "New document — choose a sample PDF" })
    .click();
  ...
  await expect(
    page.getByText("fictional-browser-record.pdf", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText(
      "fictional-browser-record.pdf was added for this session only.",
    ),
  ).toBeVisible();
...
test("renders hostile metadata as text without making off-origin requests", async ({
  page,
}) => {
  ...
  const hostileName =
    'FAKE_<img src=x onerror="window.__mycarlos_xss=true">.pdf';
  const chooserPromise = page.waitForEvent("filechooser");
  await page
    .getByRole("button", { name: "New document — choose a sample PDF" })
    .click();
  ...
  await expect
    .poll(() => page.evaluate(() => Reflect.get(window, "__mycarlos_xss")))
    .toBeUndefined();
  expect(offOriginRequests).toEqual([]);
});

test("shows the purpose of the library navigation sections", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  ...
  await expect(
    page.getByText("Bloodwork — thyroid", { exact: true }),
  ).not.toBeVisible();

  await openSection("security", "Security & backup");
  const cloudBackup = page.getByRole("button", {
  ...
  await expect(
    page.getByRole("heading", { name: "Sample health snapshot" }),
  ).toBeVisible();
  await page.screenshot({
    path: testInfo.outputPath("health.png"),
    fullPage: true,
  });
});

test("carries a record through starred, trash, and restore", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  ...
  await page.screenshot({
    path: testInfo.outputPath("record-preview.png"),
    fullPage: true,
  });
