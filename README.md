...
- Multiple patient profiles, nested folders, transactional bulk folder assignments, manual/background/
  ...
  an import or export hides content at once and locks when the transfer finishes (unless the
  transfer outlasts the native deadline, which counts it for at most its first 15 minutes). A
  native idle deadline also drops the keys shortly after the renderer's (the lock delay plus 15
  seconds after the last activity it heard of) if the webview stops responding.
