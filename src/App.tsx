...
const sampleFolders: FolderItem[] = [
  {
    ...
    subtitle: "9 items",
    date: "Renamed 2 Aug 2026",
  },
  {
    id: "folder-results",
    title: "Test results 2026",
    subtitle: "14 items",
    date: "9 Jul 2026",
  },
  {
    id: "folder-old",
    title: "Old records from Dr. Chen",
    subtitle: "11 items",
    date: "Mar 2019",
  },
];
...
export default function App({ bridge = defaultBridge }: AppProps) {
  ...
  useModalFocus(Boolean(activeDocumentId), dialogRef, () =>
  ...
  function openDocument(document: DemoDocument) {
    ...
    setRecentIds((current) =>
      [document.id, ...current.filter((id) => id !== document.id)].slice(0, 8),
    );
    ...
    setDocuments((current) =>
      current.map((candidate) =>
        candidate.id === document.id
          ? { ...candidate, starred: nextStarred }
          : candidate,
      ),
    );
    setNotice(
      `${document.title} was ${nextStarred ? "added to" : "removed from"} Starred.`,
    );
  }
  ...
  function starSelectedDocuments() {
    ...
    setDocuments((current) =>
      current.map((document) =>
        ids.has(document.id) ? { ...document, starred: true } : document,
      ),
    );
    setSelectedIds([]);
    setNotice(
      `${selectedDocuments.length} document${selectedDocuments.length === 1 ? "" : "s"} added to Starred.`,
    );
  }
  ...
  function noteDemoAction(action: string) {
    setNotice(
      `${action} is demonstrated visually; no security material was created or changed.`,
    );
  }
    ...
    <div className="evaluation-page">
      ...
      <div className="page-wrap">
        <section
          ...
          aria-label="myCarlos record library evaluation"
        >
          <header className="titlebar" inert={Boolean(activeDocument)}>
            ...
            <span className="unlock-pill">
              <Icon name="lock-open" /> Unlocked
            </span>
          </header>

          <label
            className="mobile-section-picker"
            inert={Boolean(activeDocument)}
          >
            <span>Section</span>
            <select
              aria-label="Section"
              value={activeSection}
              onChange={(event) =>
                showSection(event.target.value as AppSection)
              }
            >
          ...
          <div className="app-body" inert={Boolean(activeDocument)}>
            <aside className="sidebar">
              ...
              <nav className="side-nav" aria-label="Record library">
                <button
                  className={
                    activeSection === "records" && filter === "All"
                      ? "selected"
                      : ""
                  }
                  type="button"
                ...
                <button
                  className={activeSection === "recent" ? "selected" : ""}
                  type="button"
                  onClick={() => showSection("recent")}
                >
                ...
                <button
                  ...
                  type="button"
                  onClick={() => showSection("starred")}
                >
                  <Icon name="star" /> Starred{" "}
                  <span className="nav-count">{starredCount}</span>
                </button>
                <button
                  className={activeSection === "trash" ? "selected" : ""}
                  type="button"
                  onClick={() => showSection("trash")}
                >
                  <Icon name="trash" /> Trash{" "}
                  <span className="nav-count">{trashItems.length}</span>
                </button>

                <span className="nav-label">By kind</span>
                <button
                  className={
                    activeSection === "records" && filter === "Test results"
                      ? "selected"
                      : ""
                  }
                  type="button"
                ...
                <button
                  className={
                    activeSection === "records" && filter === "Letters"
                      ? "selected"
                      : ""
                  }
                  type="button"
                ...
                <button
                  className={
                    activeSection === "records" && filter === "Imaging"
                      ? "selected"
                      : ""
                  }
                  type="button"
                ...
                <button
                  className={
                    activeSection === "records" && filter === "Prescriptions"
                      ? "selected"
                      : ""
                  }
                  type="button"
                ...
                <button
                  className={activeSection === "security" ? "selected" : ""}
                  type="button"
                  onClick={() => showSection("security")}
                >
                  <Icon name="shield" /> Security & backup
                </button>
                <button
                  className={activeSection === "health" ? "selected" : ""}
                  type="button"
                  onClick={() => showSection("health")}
                >
                  <Icon name="heart" /> Health data
                </button>
              </nav>
            ...
            {(activeSection === "records" ||
              ...
              <main className="library-main">
                ...
                <div className="main-head">
                  ...
                  {activeSection === "records" && (
                    <div className="head-actions">
                      ...
                      >
                        <Icon name="plus" />
                        <span>{importing ? "Opening…" : "New"}</span>
                      </button>
                    </div>
                  )}
                </div>
                ...
                {view === "list" ? (
                  <div
                    className="filelist"
                    aria-label={`${visibleCount} visible library items`}
                  >
                    ...
                    {filteredDocuments.map((document) => (
                      <article className="file-row" key={document.id}>
                        ...
                        />
                        <div className="file-name">
                          <span className={`document-icon ${document.icon}`}>
                          ...
                          <span className="name-copy">
                            ...
                            <small>
                              {document.subtitle}
                              {document.sessionOnly ? " · session only" : ""}
                            </small>
                          </span>
                        </div>
            ...
            {activeSection === "trash" && (
              <main className="library-main purpose-screen">
                ...
                <div className="purpose-note">
                  <Icon name="info" />
                  <span>
                    Deleted medical records remain recoverable here for 30 days.
                    Restoring puts an item back where it was.
                  </span>
                </div>
                <p className="status-line" role="status" aria-live="polite">
                  {notice}
                ...
                <div
                  className="filelist trash-list"
                  aria-label={`${trashItems.length} items in trash`}
                >
                  <div className="file-head" aria-hidden="true">
                    <span />
                    <span className="sorted">Name</span>
                  ...
                  {trashItems.map((item) => (
                    <article className="file-row" key={item.id}>
                      <span className="trash-marker">
                        <Icon name="trash" />
                      </span>
                      <div className="file-name">
                        <span className={`document-icon ${item.icon}`}>
                          <Icon name={item.icon} />
                        </span>
                        <span className="name-copy">
                  ...
                  {trashItems.length === 0 && (
                    <div className="empty-state">
                      ...
                      <span>
                        Deleted records would remain here for 30 days.
                      </span>
                    </div>
                  )}
                </div>
            ...
            {activeSection === "security" && (
              <main className="library-main purpose-screen">
                ...
                <div className="purpose-note warning">
                  <Icon name="info" />
                  <span>
                    <strong>Evaluation controls only.</strong> These settings
                    change this screen for the current session; no encryption
                    keys or backups are created.
                  </span>
                </div>
                <p className="status-line" role="status" aria-live="polite">
                  {notice}
                ...
                <div className="setting-list">
                  <section className="setting-row">
                    <div>
                      <h2>
                        Encryption{" "}
                        <span className="state-pill">On — always</span>
                      </h2>
                      <p>
                        Your records would be scrambled on this device using
                        your passphrase. This cannot be turned off.
                      </p>
                    </div>
                  </section>
                  ...
                  <section className="setting-row">
                    <div>
                      <h2>
                        Recovery sheet{" "}
                        <span className="state-pill warning">Not created</span>
                      </h2>
                      <p>
                        A printable recovery sheet could reopen the vault if the
                        passphrase is forgotten. Printing a new one would
                        invalidate the old sheet.
                      </p>
                    </div>
                    <button
                      className="button"
                      type="button"
                      onClick={() => noteDemoAction("Recovery sheet creation")}
                    >
                  ...
                  <section className="setting-row">
                    <div>
                      <h2>
                        Backup to iCloud{" "}
                        <span
                          className={`state-pill ${cloudBackup ? "" : "off"}`}
                        >
                          {cloudBackup ? "On" : "Off"}
                        </span>
                      </h2>
                      <p>
                        The production concept copies records while they are
                        still scrambled, so the cloud provider cannot read them.
                      </p>
                    </div>
                    <button
                  ...
                  <section className="setting-row">
                    <div>
                      <h2>
                        Backup to a folder or drive{" "}
                        <span
                          className={`state-pill ${driveBackup ? "" : "off"}`}
                        >
                          {driveBackup ? "On" : "Off"}
                        </span>
                      </h2>
                      <p>
                        A second encrypted copy could be stored on a USB drive
                        or in a folder chosen by the patient.
                      </p>
                    </div>
                    <button
                  ...
                  <section className="setting-row">
                    <div>
                      <h2>
                        ...
                        <span
                          className={`state-pill ${biometricUnlock ? "" : "off"}`}
                        >
                          {biometricUnlock ? "On" : "Off"}
                        </span>
                      </h2>
                      <p>
                        Face ID, Touch ID, or the Android device credential
                        could unlock the app; the passphrase would still be
                        required after restarting.
                      </p>
                    </div>
                    <button
                  ...
                  <section className="setting-row">
                    <div>
                      ...
                      <p>
                        A real change would securely rewrap the vault master key
                        without rewriting every encrypted record.
                      </p>
                    </div>
                    <button
                      className="button"
                      type="button"
                      onClick={() => noteDemoAction("Passphrase change")}
                    >
              ...
              <main className="library-main purpose-screen">
                <div className="main-head">
                  <div>
                    <h1>Health data</h1>
                    <p>
                      Readings from your phone and devices, alongside your
                      records
                    </p>
                  </div>
                </div>
                ...
                <div className="purpose-note warning">
                  <Icon name="flask" />
                  <span>
                    <strong>Concept only — mobile devices.</strong> Apple Health
                    and Health Connect store readings such as blood pressure and
                    weight, not letters or reports. No health permission is
                    requested by this evaluation.
                  </span>
                </div>
                  ...
                  <section className="setting-row">
                    <div>
                      <h2>
                        Apple Health{" "}
                        <span
                          className={`state-pill ${appleHealth ? "" : "off"}`}
                        >
                          {appleHealth ? "Demo connected" : "Not connected"}
                        </span>
                      </h2>
                      <p>
                        On iPhone, the patient could choose to read blood
                        pressure, weight, and steps. Nothing would be written
                        back.
                      </p>
                    </div>
                    <button
                      className="button"
                      type="button"
                      aria-pressed={appleHealth}
                      onClick={() => setAppleHealth((current) => !current)}
                    >
                      {appleHealth ? "Disconnect demo" : "Connect demo"}
                    </button>
                  </section>
                  ...
                  <section className="setting-row">
                    <div>
                      <h2>
                        Health Connect{" "}
                        <span
                          className={`state-pill ${healthConnect ? "" : "off"}`}
                        >
                          {healthConnect ? "Demo connected" : "Not connected"}
                        </span>
                      </h2>
                      <p>
                        The Android equivalent could provide the same kinds of
                        readings through a separate permission.
                      </p>
                    </div>
                    <button
                      className="button"
                      type="button"
                      aria-pressed={healthConnect}
                      onClick={() => setHealthConnect((current) => !current)}
                    >
                      {healthConnect ? "Disconnect demo" : "Connect demo"}
                    </button>
                  </section>
                </div>
                ...
                {appleHealth || healthConnect ? (
                  <section
                    ...
                    aria-label="Synthetic health reading preview"
                  >
                    <div className="health-preview-heading">
                      <div>
                        <span className="eyebrow">Synthetic preview</span>
                        <h2>Sample health snapshot</h2>
                      </div>
                  ...
                  <div className="health-empty">
                    ...
                    <p>
                      A production connection could place trends beside clinic
                      documents, giving the patient one view of the whole
                      picture.
                    </p>
                  </div>
                )}
          ...
          {activeDocument && (
            <div
              ...
              role="presentation"
              onMouseDown={() => setActiveDocumentId(null)}
            >
              <section
                ref={dialogRef}
              ...
              >
                ...
                <div
                  ...
                  aria-label="Placeholder document contents"
                >
                  <span className={`document-icon ${activeDocument.icon}`}>
                    <Icon name={activeDocument.icon} />
                  </span>
                  ...
                  <p>
                    Preview placeholder — this evaluation does not read the
                    selected file.
                  </p>
                </div>
                ...
                <dl className="record-metadata">
                  ...
                  <div>
                    <dt>Sent by</dt>
                    <dd>{activeDocument.source}</dd>
                  </div>
                  <div>
                    <dt>Date added</dt>
                    <dd>{activeDocument.added}</dd>
                  </div>
                  <div>
                    <dt>File</dt>
                    <dd>{activeDocument.subtitle}</dd>
                  </div>
                </dl>
                ...
                <footer className="dialog-actions">
                  <button
                    className="button"
                    type="button"
                    onClick={() => toggleStar(activeDocument)}
                  >
                    <Icon name="star" />{" "}
                    {activeDocument.starred ? "Remove star" : "Add to Starred"}
                  </button>
                  <button
                    className="button danger"
                    type="button"
                    onClick={() => moveDocumentsToTrash([activeDocument])}
                  >
        ...
        <details className="evaluation-details">
          <summary>
            <Icon name="info" /> Evaluation details{" "}
            <span>View the Tauri boundary and safety scope</span>
          </summary>
          <div className="evaluation-content">
                ...
                <>
                  ...
                  <dl>
                    ...
                    <div>
                      <dt>Bridge</dt>
                      <dd>
                        {runtime.native ? "Rust command" : "Browser preview"}
                      </dd>
                    </div>
                  </dl>
                </>
            ...
            <section>
              ...
              <ul>
                <li>
                  The picker returns only a filename; this POC does not read the
                  file.
                </li>
                <li>
                  No accounts, CARLOS connection, encryption, backup, or
                  persistence.
                </li>
